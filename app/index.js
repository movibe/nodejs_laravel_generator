...
GenlaravelGenerator.prototype.gitIgnore = function() {
   this.copy('dbconfig.php.tmpl', 'dbconfig.php', {force: true});
};

//saving the application configuration into app.php
GenlaravelGenerator.prototype.gitIgnore = function() {
   this.copy('app.php.tmpl', 'app.php', {force:true});
};
...
GenlaravelGenerator.prototype.installComposer = function(){
    var done = this.async(), child, me = this;

    fs.chmodSync('laravel/artisan', '0755');
    exec("sed -s -i '/require/a\"way/generators\"\: \"dev-master\",' laravel/composer.json", function(error,stdout,stderr){ });
    //var cmd = "sed -s -i \"\\:WorkbenchServiceProvider:a \\'Way\\\\\\\Generators\\\\\\\GeneratorsServiceProvider\\'\"";
    //exec(cmd + " laravel/app/config/app.php", function(error,stdout,stderr){ });
    ...
    process.chdir(me.userInput.larDir);
        laravel.getComposer().on('done',function(){
            ...
            composer.on('close', function (code) {
                var install2= spawn('composer', ['update']);
                install2.stdout.on('data', function (data) { console.log('Log: ' + data); done(); });
...
GenlaravelGenerator.prototype.configDB = function(){
     ...
     this.copy('database.php.tmpl','app/config/database.php', {force: true});
     //startServer(this);
     done();
};
